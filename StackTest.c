...
Status StackPop(Stack* pStack, void* pElem) {
    if (pStack->pTop == pStack->pBase) {
        ...
        //把pStack->pTop的pStack->element给个字节内存拷贝给pElem
        //此时pElem即指向存储的内容
        return OK;
    }
}
...
int main(){
    Stack* pStack;
    pStack = StackConstruct(sizeof(char));
     pStack->elementSize =1;
    char a = 'a';
    char b = 'b';
    char c = 'c';
    StackPush(pStack, &a);
    ...
    char pData;
    printf("%c\n", StackLength(pStack));
    char d=*(char*)StackGetTop(pStack);
    printf("栈顶为：%c\n",c);
    while (!IsEmpty(pStack)) {
        StackPop(pStack, &pData);
        printf("%c\n", pData);
    }
